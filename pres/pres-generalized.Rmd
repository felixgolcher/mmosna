...
## What the f$\cdots$k is a Poisson distribution? {.build}
...
* The variance is always $\lambda$. The Poisson distribution has only one parameter.

## Set up example data | simulation
...
* Suppose we install a wild life camera in the forest.
* It is triggered `r signif(exp(rate),3)` times per hour.
...
* Let's assume it actually did and the rate is now `r signif(exp(eff[2]),3)` times as high, that is `r signif(exp(rate+eff[2]),3)` per hour.

```{r, title = "Create our simulated data", results='hide'}
expand_grid(rate=rate, eff=eff, time=time) %>% 
  mutate(cond = c("damals","heute")[as.numeric(factor(eff))],
         eta = eff+rate, # witout offset here
         h_eta = exp(eta), # still w/o offset, the resulting rate.
         lambda = h_eta*time, # =exp(eff)*exp(rate)*time <=> log(lambda) = log(time) + ... 
         resp = rpois(n(), lambda)
         ) -> dta
```
