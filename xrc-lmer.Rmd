...
w0 <- lmer(extra~group+(1|ID), sleep)
w0.noreml <- update(w0, REML = T) # but anova does this automatically...
w0.noreml.nogroup <- update(w0.noreml, .~. - group)
...
**do that with your model.** Do also add the interaction between `Abi` and `situation` and check its significance. 
